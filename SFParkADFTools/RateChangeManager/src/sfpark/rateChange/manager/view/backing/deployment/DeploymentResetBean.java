...
/**
 * Description:
 * This class provides functionality to the deploymentResetPage.jsff
 * 
 * 
 * Change History:
 ...
 * ----------- -------------------------------- --------------------------------------------------------
 * 20111130-01 Mark Piller - Oracle Consulting  Created this class file
 */
public class DeploymentResetBean extends BaseBean implements RequestScopeBeanInterface {
    ...
    public void resetButtonHandler(ActionEvent event) {
        ...
        OperationStatus operationStatus = DMLOperationsProvider.INSTANCE.resetRateChange(DTO);

        if (operationStatus.isSuccess()) {
            // Move on to the next page
            // Reuse the ERROR_TITLE and ERROR_MESSAGE variables
            setPageFlowScopeValue(PageFlowScopeKey.ERROR_TITLE.getKey(),
                                  TranslationUtil.getCommonBundleString(CommonBundleKey.string_title_reset_operation_successful));
            setPageFlowScopeValue(PageFlowScopeKey.ERROR_MESSAGE.getKey(),
                                  TranslationUtil.getCommonBundleString(CommonBundleKey.string_message_reset_rate_change_process_control_successful,
                                                                        DTO.getRateChangeReference()));
            setSessionScopeValue(SessionScopeKey.NAVIGATION_INFO.getKey(),
                                 NavigationFlow.AfterResetRateChangeProcess.name());

        } else {
            setInlineMessageText(operationStatus.getMessage());
            // setInlineMessageText(TranslationUtil.getErrorBundleString(ErrorBundleKey.error_reset_failure));
